...
class Gaussian2DModel:
    def __init__(self, N, iteration=None, lr=None, gt=False, fixed=[], gt_model=None, g_threshold=False):
        ...
        self._opacity = torch.nn.Parameter((torch.rand(N, 1).to(device) - 0.5) * 3 + 1.5)
        self.fixed = fixed
...
if __name__ == "__main__":
    ...
    model = Gaussian2DModel(N, iteration=iteration, lr=lr, fixed=fixed, gt_model=gt_model, g_threshold=True)
